...
if(_time <= 0) then { _time = time + (15 * 60); hintC "Сообщите Админу: JAIL_FALLBACK_15, time is zero!"; };
...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		{
			hintSilent format["==Начальник==Оставшееся время:\n %1 можете оплатить залог: %3 сумма залога: $%2",_countDown,[life_bail_amount] call life_fnc_numberText];
		}
		else
		{
			hintSilent format["==Начальник==Оставшееся время:\n %1\n",_countDown];
...
switch (true) do
{
	case (_bail) :
	{
		...
		titleText["Вы заплатили залог и будете освобождены.","PLAIN"];
	...
	{
		life_is_arrested = false;
		hint "Ты сбежал из тюрьмы, и находитесь в розыске. Кроме этого, вам добавилась статья за побег и когда вас поймают ваш срок будет удвоен.";
	...
	{
		life_is_arrested = false;
		titleText["Вы отсидели свое время и будете освобождены.","PLAIN"];
		[[getPlayerUID player],"life_fnc_wantedRemove",false,false] spawn life_fnc_MP;
		player setPos (getMarkerPos "jail_release");
		[5] call SOCK_fnc_updatePartial;
	};

...
switch (life_veh_shop select 0) do
{
	...
	case "aan_car": {_ret = license_civ_aan;};
	case "reb_car": {_ret = license_civ_rebel;};
	case "reb_adf": {_ret = license_civ_adf;};
	case "reb_krem": {_ret = license_civ_krem;};
	case "cop_car": {_ret = true;};
	case "sek_hq": {_ret = license_cop_sek;};
	case "adf_car": {_ret = license_civ_adf;};

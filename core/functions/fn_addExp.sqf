...
if( _exp >= _nextLevel ) then {
	...
	if( _level >= 20 ) then {
		...
		if( _level == 20 ) then {
				...
				hint parseText format["<t align='center'><t size='1.5'>ПОЗДРАВЛЕНИЕ!<br/>ТЫ СЕЙЧАС</t><br/><t color='#FFFF00'><t size='2'>LVL: %1<br/>%2<br/></t></t></t>",_level,_text];
		} else { 
				missionNamespace setVariable [_type,[(_profData select 0) + 1,(_exp)]];
				_level = (_profData select 0) + 1;
				_text = [_type] call life_fnc_varToStr;
				hint parseText format["<t align='center'><t size='1'>ПОЗДРАВЛЕНИЕ</t><br/><t color='#FFFF00'><t size='1.5'>%1</t></t><br/><t size='1.5'>СЕЙЧАС</t><br/><t color='#0088CC'><t size='1.5'>LVL: %2</t></t></t>",_text,_level];
		};

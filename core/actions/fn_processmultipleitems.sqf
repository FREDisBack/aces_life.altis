...
if (life_is_processing) exitWith {hint "Коллега занят. Пожалуйста не стрессуйте!";};    
...
if(vehicle player != player) exitwith {hint format["Ты не можешь сидеть в транспортном средстве"];};
...
_pgText ctrlSetText format["%2 (1%1)...","%","В ходе производства ."];
...
while{true} do
{
	...
	_pgText ctrlSetText format["%3 (%1%2)...",round(_cP * 100),"%","В ходе производства ."];
	if(_cP >= 1) exitWith {};
	if(player distance _processor > 10) exitWith {};
};

if(player distance _processor > 10) exitWith {hint "Вы ушли слишком далеко! Вам надо вернуться на место"; 5 cutText ["","PLAIN"]; life_is_processing = false;};

...
if(life_cash_pirates < _basePrice) exitWith {hint "У вас не хватает наличных!"};

if(isNil {_vehicleData}) exitWith {hint "Спавненый автомобиль?."};
if (!(_veh in life_vehicles)) exitwith {hint "Вы не являетесь владельцем!"};
		...
		if(player != vehicle player) exitWith {titleText["Вам надо выйти из транспортного средства тогда сможем покрасить!","PLAIN"];};

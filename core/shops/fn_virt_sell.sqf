#include <macro.h>
/*
	File: fn_virt_sell.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	Sell a virtual item to the store / shop
*/
private["_type","_index","_price","_var","_amount","_name","_marketprice"];
if((lbCurSel 2402) == -1) exitWith {};
_type = lbData[2402,(lbCurSel 2402)];
_index = -1;
_price = -1;

if(life_shop_type != "evidence") then {
_index = [_type,__GETC__(sell_array)] call TON_fnc_index;
	
if(_index == -1) exitWith {};

_price = (__GETC__(sell_array) select _index) select 1;
}
else
{
	_index = [_type,life_illegal_items] call TON_fnc_index;
	
	if(_index == -1) exitWith {};

	_price = (life_illegal_items select _index) select 1;
};

if (_price == -1) exitWith {};

_var = [_type,0] call life_fnc_varHandle;

_marketprice = [_type] call life_fnc_marketGetSellPrice;

if(_marketprice != -1) then
{
	_price = _marketprice;
};

_amount = ctrlText 2405;
if ((time - life_vshop_timer) < 3) exitWith {hint "Вы слишком быстро нажимаете на кнопку ПРОДАТЬ! Рекомендуем указывать нужное кол-во при продаже одинаковых предметов, вместо нажатия кнопки сто раз!"; life_action_inUse = false};
life_vshop_timer = time;
if(!([_amount] call TON_fnc_isnumber)) exitWith {hint "Ты не указал никакое актуальное число.";};
_amount = parseNumber (_amount);
if(_amount > (missionNameSpace getVariable _var)) exitWith {hint "Ты не имеешь так много предметов на продажу!"};

_price = (_price * _amount);
_name = [_var] call life_fnc_vartostr;
if(([false,_type,_amount] call life_fnc_handleInv)) then
{

	hint format["Вы продали %1 %2 за %3€",_amount,_name,[_price] call life_fnc_numberText];
	life_cash_pirates = life_cash_pirates + _price;
	
    if(_marketprice != -1) then 
    { 
		//##94
        [_type, _amount] spawn
        {
			sleep 120;
            [_this select 0,_this select 1] call life_fnc_marketSell;
		};
        };

        [] call life_fnc_virt_update; 
    };

if(life_shop_type == "heroin") then
{
	private["_array","_ind","_val"];
	_array = life_shop_npc getVariable["sellers",[]];
	_ind = [getPlayerUID player,_array] call TON_fnc_index;
	if(_ind != -1) then
	{
		_val = (_array select _ind) select 2;
		_val = _val + _price;
		_array set[_ind,[getPlayerUID player,profileName,_val]];
		life_shop_npc setVariable["sellers",_array,true];
	}
		else
	{
		_array pushBack [getPlayerUID player,profileName,_price];
		life_shop_npc setVariable["sellers",_array,true];
	};
};

[0] call SOCK_fnc_updatePartial;
[3] call SOCK_fnc_updatePartial;
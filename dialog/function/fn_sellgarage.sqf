...
if(life_cgar_inUse+(300) >= time) exitWith {closeDialog 0;hint format["Вы можете продать технику не более одного раза в 5 минут, следующая продажа через: %1:%2",4 - floor ((time - life_cgar_inUse) / 60),59 - round (time - life_cgar_inUse - (floor ((time - life_cgar_inUse) / 60)) * 60)];};
